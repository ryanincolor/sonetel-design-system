...
function convertToKebabCase(filePath) {
  ...
  const kebabCaseContent = fileContent.replace(/\--([a-z])({0})((?:[a-zA-Z]()[a-zA-Z0-9]*)*/g, (match, firstChar, secondChar, rest) => {
    return `--${firstChar}${secondChar.toLowerCase()}${rest}`
  })
  fs.writeFileSync(filePath, kebabCaseContent)
}

// Convert csp and scss files
